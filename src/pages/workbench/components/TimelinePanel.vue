...
function typeColorClass(type) {
    const colors = {
        ...
        cause: 'bg-rose-50 text-rose-600 border-rose-200'
    };
    return colors[type] || 'bg-zinc-150 text-zinc-600 border-zinc-200';
}
</script>
...
<template>
    <div class="flex flex-col h-full bg-white text-zinc-800 p-4 overflow-hidden gap-4">
        ...
        <header class="flex flex-col md:flex-row md:items-end justify-between border-b border-zinc-200 pb-3 shrink-0 select-none">
            <div>
                <h2 class="text-sm font-bold text-zinc-800 flex items-center gap-2">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-violet-600"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
                    时间轴
                </h2>
                <p class="text-xs text-zinc-400 mt-1">按发生顺序串联点击、路由、接口、console、异常和 logcat。</p>
            </div>
        ...
        <div v-if="!normalized.length" class="flex-1 grid place-items-center rounded-lg border border-zinc-200 bg-white text-zinc-500 text-sm shadow-2xs">
            暂无时间轴事件。复现问题后会自动出现。
        </div>
        
        <div v-else class="flex flex-1 min-h-0 gap-4 overflow-hidden">
            <!-- Left: Timeline List -->
            <div class="w-1/2 flex flex-col border border-zinc-200 bg-white rounded-lg overflow-hidden relative shadow-2xs">
                <ol class="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-0 select-none">
                    <li
                        v-for="(item, index) in normalized"
                        :key="`${item.time}-${index}`"
                        class="relative pl-6 pb-4 group cursor-pointer transition-opacity"
                        :class="selectedIndex === index ? 'opacity-100' : 'opacity-65 hover:opacity-100'"
                    ...
                    >
                        ...
                        <div class="absolute left-0 top-1.5 w-4 h-4 rounded-full border-2 bg-white transition-colors z-10"
                             :class="selectedIndex === index ? 'border-violet-600 bg-violet-50 scale-110' : 'border-zinc-300'"></div>
                        
                        <div class="flex flex-col gap-1 rounded-lg border p-2.5 transition-all"
                             :class="selectedIndex === index ? 'bg-zinc-50 border-zinc-200 shadow-3xs' : 'bg-transparent border-transparent group-hover:bg-zinc-50 group-hover:border-zinc-100'">
                            <div class="flex items-center gap-2">
                                <time class="text-xs font-mono text-zinc-400">{{ formatTime(item.time) }}</time>
                            ...
                            <span class="text-xs font-semibold text-zinc-700 line-clamp-2 leading-relaxed mt-0.5">{{ redact(item.message) }}</span>
                        </div>
                    </li>
                </ol>
            </div>
            ...
            <!-- Right: Event Detail -->
            <aside class="w-1/2 flex flex-col border border-zinc-200 bg-white rounded-lg overflow-hidden shadow-2xs">
                <div v-if="selected" class="flex flex-col h-full">
                    <header class="p-4 border-b border-zinc-200 bg-zinc-50/50 select-none">
                        ...
                        <h3 class="text-xs font-bold text-zinc-800 leading-snug break-words">{{ redact(selected.message) || '事件详情' }}</h3>
                        
                        <dl class="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-zinc-200">
                            <div>
                                <dt class="text-[10px] uppercase text-zinc-400 font-semibold mb-1">发生时间</dt>
                                <dd class="text-xs font-mono text-zinc-650">{{ formatTime(selected.time) }}</dd>
                            </div>
                            <div>
                                <dt class="text-[10px] uppercase text-zinc-400 font-semibold mb-1">事件类型</dt>
                                <dd class="text-xs text-zinc-650">{{ typeLabel(selected.type) }}</dd>
                            </div>
                        </dl>
                    </header>
                    <div class="flex-1 overflow-auto p-4 custom-scrollbar">
                        <pre class="bg-zinc-50 p-3 rounded-lg border border-zinc-150 text-xs font-mono text-zinc-650 overflow-x-auto whitespace-pre-wrap break-words leading-relaxed selection:bg-violet-200">{{ selectedRawText }}</pre>
                    </div>
                </div>
            </aside>
        </div>
